...
2. **YouTube 제목** (🔥 가장 중요!):
   - **한글**: OOOffi 패턴 중 하나를 **반드시** 사용! (40-70자)
     * 패턴 1: "듣는 순간 [감정]하게 만드는 노래 [이모지] [용도] [이모지]"
     * 패턴 2: "[시간/계절] [감정] [용도] [이모지] [장르] [이모지]"
     * 패턴 3: "[감탄] [공감] [이모지] [효과] [이모지]"
   - **영어**: 동일한 패턴을 영어로 (예: "Songs that make you feel spring instantly🌸 Perfect cafe playlist☕")
...
⚠️ **중요**: YouTube 제목은 OOOffi 3가지 패턴 중 하나를 **반드시 사용**하고, 이모지 3-5개 포함, 감정 동사 사용!`
          }
        ],
        temperature: 0.8,
        max_tokens: 1000
