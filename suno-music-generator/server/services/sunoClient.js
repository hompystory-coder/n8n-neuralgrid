...
class SunoAPIClient {
  ...
  async uploadAndCover(params) {
    try {
      const payload = {
        ...
        ...(params.title && { title: params.title }),
        ...(params.prompt && { prompt: params.prompt }),
        callBackUrl: params.callBackUrl || '' // Always include callBackUrl
      };
  ...
  /**
   * 5-1. 업로드 & 확장 (Upload and Extend) - 스타일 기반 새 음악 생성
   * @param {Object} params
   * @param {string} params.uploadUrl - 원본 음악 URL
   * @param {boolean} params.customMode - 커스텀 모드
   * @param {boolean} params.instrumental - Instrumental 모드
   ...
   * @param {string} params.prompt - 설명
   * @param {string} params.callBackUrl - 웹훅 URL
   * @param {string} params.continueAt - 지속 시간 (초)
   * @returns {Promise<Object>}
   */
  async uploadAndExtend(params) {
    try {
      const payload = {
        uploadUrl: params.uploadUrl,
        defaultParamFlag: params.defaultParamFlag !== undefined ? params.defaultParamFlag : false,
        customMode: params.customMode !== undefined ? params.customMode : false,
        instrumental: params.instrumental !== undefined ? params.instrumental : false,
        continueAt: params.continueAt !== undefined ? parseInt(params.continueAt) : 0, // 0 = 자동으로 오디오 끝에서 확장
        model: params.model || 'V5',
        callBackUrl: params.callBackUrl || ''
      };
      
      // style, title, prompt는 선택사항
      if (params.style) payload.style = params.style;
      if (params.title) payload.title = params.title;
      if (params.prompt) payload.prompt = params.prompt;

      console.log('🎸 Upload and extend:', params.uploadUrl);
      console.log('📋 Payload:', JSON.stringify(payload, null, 2));

      const response = await this.client.post('/generate/upload-extend', payload);
      ...
      if (response.data.code === 200) {
        ...
        throw new Error(response.data.msg || 'Upload and extend failed');
      }
    } catch (error) {
      console.error('❌ Upload and extend error:', error.response?.data || error.message);
      return {
        success: false,
        error: this.parseError(error)
      };
  ...
  /**
   * 5-1-1. 오디오 파일 업로드 (File Upload for Upload And Cover)
   * @param {Object} file - Multer file object
   * @param {string} uploadPath - Upload path (e.g., 'audio-samples')
   * @returns {Promise<Object>}
   */
  async uploadAudioFile(file, uploadPath = 'audio-samples') {
    try {
      ...
      console.log('📤 Uploading audio file:', file.originalname);

      const uploadBaseURL = process.env.SUNO_UPLOAD_BASE_URL || 'https://sunoapiorg.redpandaai.co';
      
      const response = await axios.post(
        `${uploadBaseURL}/api/file-stream-upload`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${this.apiKey}`,
      ...
      if (response.data.success && response.data.data) {
        console.log('✅ Upload success:', response.data.data.downloadUrl);
        return {
          success: true,
