...
**필수 포함 요소**:
1. 구체적 시간 (예: 새벽 5시 30분)
2. 구체적 장소 (예: 어두운 주방, 햇살 쏟아지는 거리)
3. 인물과 행동 (예: 직장인이 커피를 내린다)
4. 감정 변화 (예: 지친 → 평온해짐)
...
\`\`\`json
{
  "emotionalStory": "400-500자의 영화 같은 이야기... (시간, 장소, 인물, 행동, 감정 변화 포함)",
    ...
    return {
      ...
      issues: [
        ...
        { 
          ...
          emotionalStory: '주말 오후, 벚꽃이 만개한 공원에 가족과 함께 나들이 나선 직장인. 흩날리는 꽃잎 사이로 들려오는 아이들 웃음소리에 지친 일상이 잠시 잊혀진다. 따뜻한 봄바람에 마음까지 가벼워지는 순간.',
        ...
        { 
          ...
          emotionalStory: '새벽 6시, 아직 어두운 주방에서 커피를 내리는 직장인. 원두 갈리는 소리, 뜨거운 물 부어지는 소리에 하루가 시작된다. 창가에 앉아 따뜻한 커피를 마시며 혼자만의 고요한 시간을 즐긴다.',
        ...
        { 
          ...
          emotionalStory: '저녁 7시, 한강 러닝 코스에 모인 직장인들. 처음 보는 사람들이지만 함께 달리며 점점 가까워진다. 땀 흘리며 숨 가빠도, 옆 사람의 응원에 힘이 나서 끝까지 달린다. 피니시 라인에서 하이파이브하며 웃음이 터진다.',
        ...
        { id: 4, title: '비건 카페 증가, 건강한 식습관 트렌드', description: '채식 메뉴 선택하는 젊은 세대', emotionalStory: '점심시간, 새로 생긴 비건 카페에서 샐러드를 주문한 직장인. 신선한 채소와 과일이 담긴 그릇을 보며 건강해지는 기분. 가벼운 몸과 마음으로 오후 업무를 시작한다.', sensoryDetails: '신선한 채소 향, 상큼한 과일 맛, 밝은 카페 인테리어', category: '라이프', mood: '밝음', keywords: ['비건', '건강', '카페'], realNews: true, safe: true, source: '인스타그램' },
        { id: 5, title: '주말 등산 인구 증가, 산으로 떠나는 힐링', description: '자연 속에서 힐링하는 직장인들', emotionalStory: '토요일 아침, 등산복 입고 산을 오르는 직장인. 나무 사이로 들려오는 새소리, 땀 흘리며 정상에 오르면 펼쳐지는 탁 트인 풍경. 일주일간 쌓인 스트레스가 한순간에 날아간다.', sensoryDetails: '맑은 공기, 새소리, 시원한 바람', category: '라이프', mood: '밝음', keywords: ['등산', '힐링', '자연'], realNews: true, safe: true, source: '네이버뉴스' },
        { id: 6, title: '반려식물 키우기, 초보자도 쉬운 가드닝', description: '작은 화분으로 시작하는 식물 생활', emotionalStory: '주말 오후, 작은 화분에 물을 주며 새싹이 자란 것을 발견한 직장인. 매일 조금씩 자라는 식물을 보며 작은 성취감과 위로를 느낀다. 초록빛 잎이 주는 평온함에 하루가 풍요로워진다.', sensoryDetails: '초록 잎의 싱그러움, 흙 냄새, 부드러운 잎의 촉감', category: '라이프', mood: '밝음', keywords: ['식물', '가드닝', '취미'], realNews: true, safe: true, source: '인스타그램' },
...
async function generateLyricsFromIssue(issue, style, language, gender, index, previousLyrics = []) {
  ...
  try {
    ...
    const selectedIntroStyle = introStyles[(uniqueSeed + index * 7) % introStyles.length];
...
function generateRandomVerses(issue, seed, variations) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const idx = vseed % templates.length;
...
function generateRandomChorus(issue, seed) {
  const choruses = [
    ...
    `오늘도 걸어가\n${issue.keywords[2]} 속에서\n작은 행복들을\n하나씩 모아가\n\n완벽하지 않아도\n그대로 아름다워\n있는 그대로의 나\n빛나고 있어`,
  ];
  
  const index = seed % choruses.length;
...
6. 감정이 풍부하고 독창적인 스토리
7. ${index + 1}번째 곡이므로 이전 곡들과 **완전히 다른 독특한 관점과 이야기**
8. 구체적인 이미지와 감각적 표현 사용
9. 가사만 출력 (설명이나 주석 없이)
10. **Intro와 Outro를 반드시 포함**해서 완전한 곡 구조 만들기
11. **절대로 이전 곡과 같은 표현이나 문장을 재사용하지 마세요**
